...
foreach ($arrListing as $sn => $row){
	...
	foreach ($arr_flds as $key=>$val){
		...
		switch ($key){
			...
			break;
			case 'name':
				$variantStr = '<div class="item-yk-head-title">'.wordwrap($row['product_name'],150,"<br>\n").'</div>';
				$variantStr .= ( $row['selprod_title'] != '') ? '<div class="item-yk-head-sub-title">' . wordwrap($row['selprod_title'],150,"<br>\n").'</div>' : '';
				if( is_array($row['options']) && count($row['options']) ){
					foreach($row['options'] as $op){
						$variantStr .= '<div class="item-yk-head-specification">'.wordwrap($op['option_name'].': '.$op['optionvalue_name'],150,"<br>\n").'</div>';
					}
				}
