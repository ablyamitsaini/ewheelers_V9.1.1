...
(function() {
	...
	var rpdv = '#productRentalInventory';
	
	rentalInventoryUpdateForm = function(){				
		$(rpdv).html(fcom.getLoader());
		fcom.ajax(fcom.makeUrl('Seller', 'rentalInventoryUpdateForm'), '', function(t) {			
			$(rpdv).html(t);
		});
	};
...
$(document).on('click', '.rentalCsvFile-Js', function() {
	...
	timer = setInterval(function() {
		...
		$val = $(node).val();
		$.ajax({
			url: fcom.makeUrl('Seller', 'updateRentalInventory'),
			type: 'post',
			dataType: 'json',
			data: new FormData($('#form-upload')[0]),
			cache: false,
