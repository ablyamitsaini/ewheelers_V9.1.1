		...
		<td colspan="2" class="nopadding">
			...
			foreach ($shipping_rates as $shipping) { ?>
					...
					<tr id="shipping-row<?php echo $shipping_row; ?>">
						...
						<td>
							<input type="text" name="product_shipping[<?php echo $shipping_row; ?>][company_name]" value="<?php echo $shipping["scompany_name"]?>" placeholder="<?php echo Labels::getLabel('LBL_Company',$siteLangId)?>" /><input type="hidden" name="product_shipping[<?php echo $shipping_row; ?>][company_id]" value="<?php echo $shipping["pship_company"]?>" /></td>
						<td>
							<input type="text" name="product_shipping[<?php echo $shipping_row; ?>][processing_time]" value="<?php echo ShippingDurations::getShippingDurationTitle($shipping,$siteLangId)?>" placeholder="<?php echo Labels::getLabel('LBL_Processing_Time',$siteLangId)?>" /><input type="hidden" name="product_shipping[<?php echo $shipping_row; ?>][processing_time_id]" value="<?php echo $shipping['pship_duration']?>" /></td>
						<td><input type="text" name="product_shipping[<?php echo $shipping_row; ?>][cost]" value="<?php echo $shipping["pship_charges"]?>" placeholder="<?php echo Labels::getLabel('LBL_Cost',$siteLangId)?>" /></td>
						<td>
							<input type="text" name="product_shipping[<?php echo $shipping_row; ?>][additional_cost]" value="<?php echo $shipping["pship_additional_charges"]?>" placeholder="<?php echo Labels::getLabel('LBL_Each_Additional_Item',$siteLangId)?>" /></td>
						<td><button type="button" onclick="$('#shipping-row<?php echo $shipping_row; ?>').remove();" class="btn btn--secondary ripplelink" title="<?php echo Labels::getLabel('LBL_Remove',$siteLangId)?>"><i class="fa fa-minus"></i></button>
							<!--<a class="button red medium" onclick="$('#shipping-row<?php echo $shipping_row; ?>').remove();"  title="Remove">Remove</a>-->
						</td>
					</tr>
					<?php $shipping_row++; ?>
					<?php }
				...
				<script>
					...
					addShipping = function() {
						...
						html += "</td>";
						html += "  <td><button type='button' class='btn btn--secondary ripplelink' title='<?php echo Labels::getLabel('LBL_Remove',$siteLangId)?>' onclick='removeShippingRow(" + shipping_row + ")' ><i class='fa fa-minus'></i></button></td>";
						html += "</tr>";
