...
if (!empty($orderDetail['opd_sold_or_rented']) && $orderDetail['opd_sold_or_rented'] == 2 ) {
    $rentalOrder = 1;
}
//echo "aaaaa<pre>"; print_r($order_detail); echo "</pre>"; exit;
if (!$print) {
                    ...
                    <table class="table table--orders">
                        <tbody>
                            <tr class="">
                                ...
                                <?php if (1 == $rentalOrder) { ?>
                                    <th><?php echo Labels::getLabel('LBL_Rental_Detail', $siteLangId);?></th>
                                <?php } ?>
                            ...
                            <tr>
                                ...
                                <?php if (1 == $rentalOrder) { ?>
                                    <td><?php 
                                            $rental_security += $orderDetail['opr_rental_security'];
                                            $unit_price_sufix = $short_rental_types[$orderDetail['opr_rental_type']];
                                            $opr_rental_price_multiplier = $orderDetail['opd_rental_price_multiplier'];
                                            if ($orderDetail['opd_order_type'] == 2) {
                                                $rental_duration = Labels::getLabel('LBL_Hour', $siteLangId);
                                            } else {
                                                $rental_duration = Labels::getLabel('LBL_Day', $siteLangId);
                                            }
                                        echo Labels::getLabel('LBL_Duration', $siteLangId) . ': ' .$opr_rental_price_multiplier.' '.$rental_duration; ?><br/>
                                        
                                        <small><?php echo Labels::getLabel('LBL_From', $siteLangId) . ': ' . FatDate::format($orderDetail['opd_rental_start_date'], true); ?></small><br/>
                                        <small><?php echo Labels::getLabel('LBL_To', $siteLangId) . ': ' . FatDate::format($orderDetail['opd_rental_end_date'], true); ?></small><br/>
                                        <small><?php echo Labels::getLabel('LBL_Duration_Discount', $siteLangId) . ': ' . $orderDetail['opd_rental_duration_discount']; ?>%</small><br/><br/>
                                        
                                        <?php echo Labels::getLabel('LBL_Rental_Security', $siteLangId) . ': ' .CommonHelper::displayMoneyFormat($orderDetail['opd_rental_security']); ?><br/>
                                                <small>(<?php echo Labels::getLabel('M_Refundable_after_item_returned', $siteLangId); ?>)</small>
