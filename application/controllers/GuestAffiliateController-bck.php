	...
	public function setupAffiliateRegister(){
		...
		switch($affiliate_register_step_number){
			case UserAuthentication::AFFILIATE_REG_STEP1:
				...
				$this->set( 'userAffilaiteData', json_encode($post));
				UserAuthentication::setSessionAffiliateRegistering( array( 
		...
		switch( $affiliate_register_step_number ){
			...
			case UserAuthentication::AFFILIATE_REG_STEP2:
				...
				/* $frmData = array_merge( $userExtraData, $userData ); */
				 CommonHelper::printArray($post['userAffilaiteData']);
				CommonHelper::printArray(unserialize($post['userAffilaiteData']));  
				/* if ($post == false) {
					Message::addErrorMessage(current($registerForm->getValidationErrors()));
					FatUtility::dieJsonError( Message::getHtml());				
				} */
				unset($post['btn_submit']);
				$registerForm->fill($post['userAffilaiteData']);
			break;
	...
	private function getAffiliateRegistrationForm( $registeration_step_number = UserAuthentication::AFFILIATE_REG_STEP1 ){
		...
		switch($registeration_step_number){
			...
			case UserAuthentication::AFFILIATE_REG_STEP2:
				...
				$fld = $frm->addSelectBox( Labels::getLabel('LBL_Country',$siteLangId),'user_country_id',$countriesArr, FatApp::getConfig('CONF_COUNTRY'));
				...
				$frm->addHiddenField('','credential_username');
				$frm->addHiddenField('','credential_email');
			...
			case UserAuthentication::AFFILIATE_REG_STEP3:
				...
				$frm->addHiddenField('','user_password');
				$frm->addHiddenField('','user_phone');
				$frm->addHiddenField('','uextra_company_name');
				$frm->addHiddenField('','uextra_website');
				$frm->addHiddenField('','user_address1');
				$frm->addHiddenField('','user_address2');
				$frm->addHiddenField('','user_country_id');
				$frm->addHiddenField('','user_state_id');
				$frm->addHiddenField('','user_city');
				$frm->addHiddenField('','user_zip');
				$frm->addSubmitButton('&nbsp;', 'btn_submit', Labels::getLabel('LBL_Register',$siteLangId));
				$frm->setFormTagAttribute('onsubmit', 'setupAffiliateRegister(this); return(false);');
