...
class AdvertiserController extends LoggedUserController {
	...
	public function promotionUpload(){
		...
		if(!$res = $fileHandlerObj->saveImage($_FILES['file']['tmp_name'],$attachedFileType, $recordId, 0,
	...
	public function promotionForm($promotionId = 0){
		...
		if($promotionId){
			...
			$srch->addCondition('promotion_id','=',$promotionId);
			$srch->addCondition('promotion_user_id','=',$userId);
			$srch->addMultipleFields(array('promotion_id','promotion_identifier','promotion_user_id','promotion_type','promotion_budget','promotion_cpc','promotion_duration','promotion_start_date','promotion_end_date','promotion_start_time','promotion_end_time','promotion_active','promotion_approved','banner_url','banner_target','banner_blocation_id','slide_url','slide_target'));
			$rs = $srch->getResultSet();
			$promotionDetails = FatApp::getDb()->fetch($rs);
	...
	public function analytics($promotionId = 0){
		...
		$srch->addCondition('promotion_id','=',$promotionId);
		$srch->addCondition('promotion_user_id','=',$userId);
		$srch->addMultipleFields(array('promotion_id','promotion_type','ifnull(promotion_name,promotion_identifier)as promotion_name'));
		$rs = $srch->getResultSet();
		$promotionDetails = FatApp::getDb()->fetch($rs);
	...
	private function getPromotionForm($promotionId = 0){
		...
		if(User::isSeller()){
			...
			$frm->addTextBox(Labels::getLabel('LBL_CPC'.'_['.commonHelper::getDefaultCurrencySymbol().']',$this->siteLangId), 'promotion_shop_cpc',FatApp::getConfig('CONF_CPC_SHOP',FatUtility::VAR_FLOAT,0),array('readonly'=>true));
			/*]*/
			...
			$frm->addTextBox(Labels::getLabel('LBL_CPC'.'_['.commonHelper::getDefaultCurrencySymbol().']',$this->siteLangId), 'promotion_product_cpc',FatApp::getConfig('CONF_CPC_PRODUCT',FatUtility::VAR_FLOAT,0),array('readonly'=>true));	
			/* ]*/
			...
			$frm->addTextBox(Labels::getLabel('LBL_CPC'.'_['.commonHelper::getDefaultCurrencySymbol().']',$this->siteLangId), 'promotion_slides_cpc',FatApp::getConfig('CONF_CPC_SLIDES',FatUtility::VAR_FLOAT,0),array('readonly'=>true));	
		...
		$fld = $frm->addRequiredField(Labels::getLabel('LBL_promotion_start_time',$this->siteLangId) , 'promotion_start_time', '', array('class'=>'time' ,'readonly'=>'readonly' ));
	...
	private function getPromotionSearchForm($langId){	
		...
		$frm->addDateField('','date_to','',array('readonly'=>'readonly','class'=>'field--calender','placeholder'=>Labels::getLabel('LBL_Date_To',$langId)));
