...
class ImportExportController extends LoggedUserController {
	...
	private function getImportForm($langId){
		...
		$fld = $frm->addRadioButtons('','export_option',
				$options,'',array('class'=>'list--vertical'),array('onClick'=>'importForm(this.value)'));
	...
	private function getExportForm($langId){
		...
		$fld = $frm->addRadioButtons('','export_option',
				$options,'',array('class'=>'list--vertical'),array('onClick'=>'exportForm(this.value)'));
	...
	private function getImportExportForm($langId,$type = 'EXPORT',$actionType){
		...
		switch(strtoupper($type)){
			...
			case 'IMPORT':
				...
				$fldImg->htmlAfterField = "<small>".Labels::getLabel('MSG_Invalid_data_will_not_be_processed',$langId)."</small>";
