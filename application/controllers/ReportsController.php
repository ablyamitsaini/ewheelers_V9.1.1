...
class ReportsController extends LoggedUserController {
	...
	public function searchSalesReport( $export = "" ){
		...
		if( $export == "export" ){
			...
			$sheetData = array();
			if ( empty($orderDate) ) {
				$arr = array(Labels::getLabel('LBL_Date', $this->siteLangId), Labels::getLabel('LBL_No._of_Orders', $this->siteLangId), Labels::getLabel('LBL_No._of_Qty', $this->siteLangId), Labels::getLabel('LBL_Refunded_Qty', $this->siteLangId), Labels::getLabel('LBL_Order_Net_Amount', $this->siteLangId), Labels::getLabel('LBL_Tax_Charged', $this->siteLangId), Labels::getLabel('LBL_Shipping_Charges', $this->siteLangId), Labels::getLabel('LBL_Refunded_Amount', $this->siteLangId), Labels::getLabel('LBL_Sales_Earnings', $this->siteLangId));
			}else{
			...
			if ( empty($orderDate) ) {
				...
				while( $row = FatApp::getDb()->fetch($rs) ){
					$arr = array( $row['op_invoice_number'], $row['totQtys'], $row['totRefundedQtys'], $row['orderNetAmount'], $row['taxTotal'], $row['shippingTotal'], $row['totalRefundedAmount'], $row['totalSalesEarnings'] );
					array_push($sheetData,$arr);
				}
			}
