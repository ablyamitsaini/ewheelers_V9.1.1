...
class CollectionsController extends MyAppController {
	...
	public function search()
	{
		...
		'theprice', 'selprod_price','selprod_stock', 'IF(selprod_stock > 0, 1, 0) AS in_stock', 'selprod_condition','prodcat_id','IFNULL(prodcat_name, prodcat_identifier) as prodcat_name','ifnull(sq_sprating.prod_rating,0) prod_rating ','selprod_sold_count', 'product_image_updated_on') );
		...
		switch( $collection['collection_type'] ){
			...
			case Collections::COLLECTION_TYPE_SHOP:
				...
				$shopObj->addMultipleFields(array( 'shop_id','shop_user_id','shop_ltemplate_id', 'shop_created_on', 'shop_name', 'shop_description', 
