...
class AccountController extends LoggedUserController
{
        ...
        if (FatApp::getConfig('CONF_ADD_FAVORITES_TO_WISHLIST', FatUtility::VAR_INT, 1) == applicationConstants::NO) {
            ...
            if ($wishLists && false ===  MOBILE_APP_API_CALL) {
                ...
                foreach ($wishLists as &$wishlist) {
                    ...
                    $srch->setPageSize(4);
                    $srch->addMultipleFields(array( 'selprod_id', 'IFNULL(selprod_title  ,IFNULL(product_name, product_identifier)) as selprod_title', 'product_id', 'IFNULL(product_name, product_identifier) as product_name', 'IF(selprod_stock > 0, 1, 0) AS in_stock','selprod_test_drive_enable'));
    ...
    public function searchWishListItems()
    {
        ...
        $srch->addMultipleFields(
            ...
            'CASE WHEN splprice_selprod_id IS NULL THEN 0 ELSE 1 END AS special_price_found', 'selprod_price', 'selprod_user_id', 'selprod_code', 'selprod_sold_count', 'selprod_condition', 'IFNULL(uwlp.uwlp_selprod_id, 0) as is_in_any_wishlist','IFNULL(uwlp.uwlp_uwlist_id, 0) as uwlp_uwlist_id','ifnull(prod_rating,0) prod_rating', 'selprod_min_order_qty' ,'selprod_test_drive_enable','selprod_book_now_enable','product_book' )
        );
    ...
    public function searchFavouriteListItems()
    {
        ...
        $srch->addMultipleFields(
            ...
            'IF(selprod_stock > 0, 1, 0) AS in_stock', 'brand.brand_id', 'product_model',
            'IFNULL(brand_name, brand_identifier) as brand_name', 'IFNULL(splprice_price, selprod_price) AS theprice','splprice_display_list_price', 'splprice_display_dis_val','splprice_display_dis_type',
            'CASE WHEN splprice_selprod_id IS NULL THEN 0 ELSE 1 END AS special_price_found', 'selprod_price', 'selprod_user_id', 'selprod_code', 'selprod_condition', 'IFNULL(uwlp.uwlp_selprod_id, 0) as is_in_any_wishlist', 'ifnull(prod_rating,0) prod_rating','selprod_sold_count','selprod_test_drive_enable' )
        );
