...
class SellerController extends LoggedUserController {	
	...
	public function viewOrder($op_id){
	...
	public function setUpCatalogRequest(){
		...
		if (is_uploaded_file($_FILES['file']['tmp_name'])) {
			...
			if(!$res = $fileHandlerObj->saveAttachment($_FILES['file']['tmp_name'],AttachedFile::FILETYPE_SELLER_CATALOG_REQUEST, $scatrequest_id, 0,	$_FILES['file']['name'], -1, $unique_record = true)){
	...
	public function catalogRequestMessageSearch(){
		...
		$srch->addMultipleFields( array( 'scatrequestmsg_id','scatrequestmsg_from_user_id', 'scatrequestmsg_from_admin_id', 
	...
	public function uploadShopImages(){
		...
		$fileHandlerObj = new AttachedFile();
		if(!$res = $fileHandlerObj->saveImage($_FILES['file']['tmp_name'], $file_type, $shop_id, 0,
		$_FILES['file']['name'], -1, $unique_record, $lang_id)
		){
			Message::addErrorMessage($fileHandlerObj->getError());
			FatUtility::dieJsonError( Message::getHtml() );
		}
	...
	public function setUpCategoryBanner(){	
		...
		$fileHandlerObj = new AttachedFile();
		if(!$res = $fileHandlerObj->saveAttachment($_FILES['file']['tmp_name'], AttachedFile::FILETYPE_CATEGORY_BANNER_SELLER, $shop_id, $prodCatId,
	...
	public function orderReturnRequestSearch(){
		...
		$srch->setPageNumber($page);
		$srch->setPageSize($pagesize);
		$srch->addMultipleFields( array( 'orrequest_id', 'orrequest_user_id', 'orrequest_qty', 'orrequest_type', 'orrequest_reference', 'orrequest_date', 'orrequest_status',
	...
	private function getShopLogoForm($shop_id,$langId){
		...
		$fld = $frm->addButton(Labels::getLabel('Lbl_Logo',$langId),'shop_logo',Labels::getLabel('LBL_Upload_Logo',$this->siteLangId),
	...
	private function getBackgroundImageForm($shop_id,$langId){
		...
		$fld = $frm->addButton(Labels::getLabel('Lbl_Background_Image',$langId),'shop_background_image',Labels::getLabel('LBL_Upload_Background_Image',$this->siteLangId),
	...
	private function getShopBannerForm($shop_id,$langId){
		...
		$fld1 =  $frm->addButton(Labels::getLabel('Lbl_Banner',$langId),'shop_banner',Labels::getLabel('LBL_Upload_Banner',$this->siteLangId),
	...
	public function sellerShippingForm($productId){
		...
		$srch = Product::getSearchObject($this->siteLangId);
		$srch->addMultipleFields( array('product_id', 'product_seller_id', 'product_added_by_admin_id',
		'IFNULL(product_name,product_identifier)as product_name') );
	...
	function productLinks($product_id){
	...
	private function getCustomProductForm($type = 'CUSTOM_PRODUCT',$prodcat_id = 0){
		...
		$frm->addFloatField( Labels::getLabel('LBL_Minimum_Selling_Price', $this->siteLangId).' ['.CommonHelper::getCurrencySymbol(true).']', 'product_min_selling_price', '');
	...
	private function getSellerProductForm( $product_id ,$type = 'SELLER_PRODUCT' ){
		...
		$frm->addIntegerField( Labels::getLabel('LBL_Minimum_Quantity',$this->siteLangId),'selprod_min_order_qty');
