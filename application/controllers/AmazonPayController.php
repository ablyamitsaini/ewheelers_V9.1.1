...
class AmazonPayController extends PaymentController{
	...
	public function get_details($orderId){
		if(strtolower($_SERVER['REQUEST_METHOD']) != 'post') FatUtility::dieJsonError( Labels::getLabel('MSG_INVALID_POST_REQUEST',$this->siteLangId) );
		$postedData = FatApp::getPostedData();
		if(!isset($postedData['orderReferenceId']) && !isset($postedData['addressConsentToken'])) FatUtility::dieJsonError( Labels::getLabel('MSG_INVALID_POST_REQUEST',$this->siteLangId) );
		elseif(strlen($postedData['orderReferenceId']) <= 0) FatUtility::dieJsonError( Labels::getLabel('MSG_INVALID_POST_REQUEST',$this->siteLangId) );
		$this->paymentSettings = $this->getPaymentSettings();
		$config = array(
			'merchant_id'		=> trim($this->paymentSettings['amazon_merchantId']),
			'access_key'		=> trim($this->paymentSettings['amazon_accessKey']),
		...
		if ( $orderInfo && $orderInfo["order_is_paid"] == Orders::ORDER_IS_PENDING ) {
			...
			if(!class_exists('\PayWithAmazon\Client')) FatUtility::dieJsonError( Labels::getLabel('AMAZON_INVALID_PAYMENT_GATEWAY_SETUP_ERROR',$this->siteLangId) );
		...
		}else FatUtility::dieJsonError( Labels::getLabel('MSG_INVALID_ORDER_PAID_CANCELLED',$this->siteLangId) );
	}
	public function doPayment($orderId){
		if(strtolower($_SERVER['REQUEST_METHOD']) != 'post') FatUtility::dieJsonError( Labels::getLabel('MSG_INVALID_POST_REQUEST',$this->siteLangId) );
		$postedData = FatApp::getPostedData( );
		if(!isset($postedData['amazon_order_reference_id'])) FatUtility::dieJsonError( Labels::getLabel('MSG_INVALID_POST_REQUEST',$this->siteLangId) );
		elseif(strlen($postedData['amazon_order_reference_id']) <= 0) FatUtility::dieJsonError( Labels::getLabel('MSG_INVALID_POST_REQUEST',$this->siteLangId) );
		$this->paymentSettings = $this->getPaymentSettings();
		$config = array(
			'merchant_id'		=> trim($this->paymentSettings['amazon_merchantId']),
			'access_key'		=> trim($this->paymentSettings['amazon_accessKey']),
			...
			if($client->success) {
				...
				if($client->success) {
					...
					if($client->success) {
						...
						if($client->success) FatUtility::dieJsonSuccess( Labels::getLabel('AMAZON_PAYMENT_COMPLETE',$this->siteLangId) );
					}
				}
			}
