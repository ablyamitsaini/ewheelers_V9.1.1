...
trait Options{
	...
	private function getForm($option_id=0){
		...
		$frm->addRequiredField(Labels::getLabel('LBL_OPTION_IDENTIFIER',$this->siteLangId), 
		'option_identifier');
		
		$languages = Language::getAllNames();
		foreach($languages as $langId=>$langName){		
			$fld = $frm->addRequiredField(Labels::getLabel('LBL_OPTION_NAME',$this->siteLangId).' '.$langName, 
			'option_name'.$langId);			
		...
		$frm->addSelectBox(Labels::getLabel('LBL_OPTION_HAVE_SEPARATE_IMAGE',$this->siteLangId),'option_is_separate_images',
		$yesNoArr,0,array(),'')->requirements()->setRequired();
