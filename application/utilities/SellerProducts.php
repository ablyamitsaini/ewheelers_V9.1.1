...
trait SellerProducts{
	...
	public function sellerProductForm( $product_id, $selprod_id = 0 ){
		...
		if( !$vendorReturnAddress ){
			...
			FatApp::redirectUser(CommonHelper::generateUrl('account','profileInfo',array(USER::RETURN_ADDRESS_ACCOUNT_TAB)));
		}
		...
		foreach($languages as $langId =>$langName ){
			...
			if( !$vendorReturnAddress ){
				...
				FatApp::redirectUser(CommonHelper::generateUrl('account','profileInfo',array(USER::RETURN_ADDRESS_ACCOUNT_TAB,$langId)));
			}
		}
		if( !$product_id ){
			Message::addErrorMessage(Labels::getLabel('MSG_Invalid_Request',$this->siteLangId));
	...
	private function getSellerProductVolumeDiscountForm( $langId ){
		...
		$frm->addFloatField( Labels::getLabel("LBL_Discount_in_(%)", $this->siteLangId), "voldiscount_percentage" );
	...
	function setupSellerProductLinks(){
