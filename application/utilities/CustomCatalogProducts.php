...
trait CustomCatalogProducts{
	...
	public function customCategoryListing(){
		...
		echo FatUtility::dieJsonSuccess($result); exit;
	}
	...
	public function searchCategory($prodRootCatCode = false){
		...
		$rootCategories = $prodCatObj->getProdRootCategoriesWithKeyword($this->siteLangId, $keyword ,false, false );
	...
	private function getCustomProductImagesFrm( $preq_id = 0, $lang_id = 0 ){
		...
		$fldImg->htmlAfterField='<label class="filelabel">'.Labels::getLabel('LBL_Browse_File',$this->adminLangId).'</label></div><small>'.Labels::getLabel('LBL_Please_keep_image_dimensions_greater_than_500_x_500._You_can_upload_multiple_photos_from_here',$this->siteLangId).'</small>';
