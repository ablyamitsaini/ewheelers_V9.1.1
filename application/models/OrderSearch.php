...
class OrderSearch extends SearchBase {
	...
	public function __construct( $langId = 0 ) {
		...
		if ( $this->langId > 0) {
			$this->joinTable( Orders::DB_TBL_LANG, 'LEFT OUTER JOIN',
			'orderlang_order_id = o.order_id
			AND orderlang_lang_id = ' . $this->langId, 'o_l');
		}
	}
	...
	public function joinPackage($langId= 0){
		...
		if ( $this->langId > 0) {
			...
			$this->joinTable( SellerPackages::DB_TBL_LANG, 'LEFT OUTER JOIN',
			'sp_l.'.SellerPackages::DB_TBL_LANG_PREFIX.'spackage_id = sp.'.SellerPackages::DB_TBL_PREFIX.'id
			AND sp_l.'.SellerPackages::DB_TBL_LANG_PREFIX.'lang_id = ' . $langId, 'sp_l');
		}
	}
}
?>