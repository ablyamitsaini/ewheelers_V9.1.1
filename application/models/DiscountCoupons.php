...
class DiscountCoupons extends MyAppModel{
	...
	public static function getCouponPlans($coupon_id,$lang_id = 0){
		...
		$srch->joinTable( SellerPackagePlans::DB_TBL, 'LEFT OUTER JOIN', 'spplan_id = ctplan_spplan_id','spp');
			$srch->joinTable( SellerPackages::DB_TBL , 'LEFT OUTER JOIN',
		'sp.spackage_id = spp.spplan_spackage_id ', 'sp');
		$srch->addMultipleFields( array("spplan_id","spackage_identifier"));
		if( $lang_id ){
		
			$srch->joinTable( SellerPackages::DB_TBL . '_lang', 'LEFT OUTER JOIN',
			'spl.spackagelang_spackage_id = sp.spackage_id AND spl.spackagelang_lang_id = ' . CommonHelper::getlangId(), 'spl');
	...
	static function getPlanTitle($sPlanRow = array(), $siteLangId =0){
		if(empty($sPlanRow)){
			return '';
		}
