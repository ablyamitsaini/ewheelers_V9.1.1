...
class Report extends MyAppModel
{
    public static function salesReportObject($langId = 0, $joinSeller = false, $attr = array())
    {
        ...
        if (empty($attr)) {
            $srch->addMultipleFields(array('DATE(order_date_added) as order_date','count(op_id) as totOrders','SUM(op_qty) as totQtys','SUM(op_refund_qty) as totRefundedQtys','SUM(op_qty - op_refund_qty) as netSoldQty','sum((op_commission_charged - op_refund_commission)) as totalSalesEarnings','sum(op_refund_amount) as totalRefundedAmount','op.op_qty','op.op_product_amount_without_book','op.op_unit_price','op.op_unit_cost','SUM( op.op_unit_cost * op_qty ) as inventoryValue','IFNULL(op_other_charges, 0) as op_other_charges','sum(( op_product_amount_without_book * op_qty ) + IFNULL(op_other_charges, 0) - op_refund_amount) as orderNetAmount','(SUM(optax.opcharge_amount)) as taxTotal','(SUM(opship.opcharge_amount)) as shippingTotal'));
        } else {
