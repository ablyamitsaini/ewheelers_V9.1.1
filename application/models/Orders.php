...
class Orders extends MyAppModel
{
			...
			foreach ($subOrders as $subkey => $subval) {
				if ($subval["opd_extend_from_op_id"] > 0) { //== Mark Extended order default status delivered after payments
					...
					if($subval['op_is_booking'] == 1 && $is_paid != 1) {
						$orderStatus = FatApp::getConfig("CONF_DEFAULT_BOOKING_ORDER_STATUS");
					}
					/* ---- */
				}
    ...
    public function getOrderProductsByOpId($op_id, $langId)
    {
        ...
        $srch->joinTable(Orders::DB_TBL_ORDER_PRODUCTS_SHIPPING, 'LEFT OUTER JOIN', 'ops.opshipping_op_id = op.op_id', 'ops');
        $srch->addMultipleFields(array('op.*','opst.*','op_l.*','o.order_id','o.order_is_paid','o.order_have_booking','o.order_language_id','o.order_user_id','sum('.OrderProduct::DB_TBL_CHARGES_PREFIX.'amount) as op_other_charges', 'o.order_affiliate_user_id','pmethod_code','optsu_user_id','ops.opshipping_by_seller_user_id', 'opd.*'));
