...
class PromotionSearch extends SearchBase{
	...
	public function __construct( $langId = 0){
		...
		if ( $langId > 0) {
			$this->joinTable( Promotion::DB_LANG_TBL, 'LEFT OUTER JOIN',
	...
	public function joinSlides($langId = 0, $type = Promotion::TYPE_SLIDES , $alias ='sl'){
		...
		$this->joinTable( Slides::DB_TBL, 'LEFT OUTER JOIN',
	...
	public function joinBannersAndLocation($langId = 0 , $type = Promotion::TYPE_BANNER , $alias ='b'){
		...
		$this->joinTable( Banner::DB_TBL, 'LEFT OUTER JOIN',
            'pr.promotion_type = '.$type.' and '.$alias.'.banner_type = '.Banner::TYPE_PPC.' and '.$alias.'.banner_record_id = pr.promotion_id',$alias);
		
		if ( $langId > 0) {
			$this->joinTable( Banner::DB_LANG_TBL, 'LEFT OUTER JOIN',
            $alias.'_l.bannerlang_banner_id = '.$alias.'.banner_id and '.$alias.'_l.bannerlang_lang_id = '.$langId,$alias.'_l');
		}		
	...
	public function joinShops($langId = 0, $isActive = true, $isDisplayStatus = true){
		...
		if ( $langId > 0) {
			$this->joinTable(SHOP::DB_TBL_LANG,'LEFT OUTER JOIN',
			's_l.'.SHOP::DB_TBL_LANG_PREFIX.'shop_id = s.'.SHOP::tblFld('id').' and 
			s_l.'.SHOP::DB_TBL_LANG_PREFIX.'lang_id = '.$langId,'s_l');
		}
		
		if( $isActive ){
			$this->addCondition( 's.shop_active', '=', applicationConstants::ACTIVE );
		}
		
		if( $isDisplayStatus ){
			$this->addCondition( 's.shop_supplier_display_status', '=', applicationConstants::ON );
		}
	}
	...
	public function joinProducts($langId = 0){
		...
		if($langId){
			...
			sp_l.'.SellerProduct::DB_LANG_TBL_PREFIX.'lang_id = '.$langId,'sp_l');
			
			$this->joinTable( Product::DB_LANG_TBL, 'LEFT OUTER JOIN',
