...
class CollectionSearch extends SearchBase
{
    ...
    public function __construct( $langId = 0 ) 
    {
        ...
        if ($this->langId > 0) {
            $this->joinTable(
                Collections::DB_TBL_LANG, 'LEFT OUTER JOIN',
                'c_l.'.Collections::DB_TBL_LANG_PREFIX.'collection_id = c.'.Collections::tblFld('id').' and 
			c_l.'.Collections::DB_TBL_LANG_PREFIX.'lang_id = '.$this->langId, 'c_l' 
            );
        }
    ...
    public function joinCollectionProducts()
    {
        ...
        $this->joinTable(
            Collections::DB_TBL_COLLECTION_TO_SELPROD, 'LEFT OUTER JOIN',
            'ctsp.'.Collections::DB_TBL_COLLECTION_TO_SELPROD_PREFIX.'collection_id = c.'.Collections::tblFld('id'), 'ctsp'
        );    
    }
    ...
    public function joinCollectionCategories()
    {
        ...
        $this->joinTable(
            Collections::DB_TBL_COLLECTION_TO_PRODUCT_CATEGORIES, 'LEFT OUTER JOIN',
    ...
    public function joinCollectionShops()
    {
        ...
        $this->joinTable(
            Collections::DB_TBL_COLLECTION_TO_SHOPS, 'LEFT OUTER JOIN',
