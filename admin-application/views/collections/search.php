		...
		switch ($key){
			...
			if( $canEdit ){
				...
				$innerLiEdit->appendElement('a', array('href'=>'javascript:void(0)', 'class'=>'button small green', 
					'title'=>Labels::getLabel('LBL_Edit',$adminLangId),"onclick"=>"editCollectionFormNew(".$row['collection_id'].")"),Labels::getLabel('LBL_Edit',$adminLangId), 
				true);
				...
				if( $row['collection_type'] == Collections::COLLECTION_TYPE_PRODUCT ){
					...
					$innerLiCollectionTypeProduct->appendElement('a', array('href'=>'javascript:void(0)', 'class'=>'button small green', 
						'title'=>Labels::getLabel('LBL_Seller_Products',$adminLangId),"onclick"=>"selprodForm(".$row['collection_id'].")"),Labels::getLabel('LBL_Seller_Products',$adminLangId), 
					true);
				}
				...
				if( $row['collection_type'] == Collections::COLLECTION_TYPE_CATEGORY ){
					...
					$innerLiCollectionTypeCat->appendElement('a', array('href'=>'javascript:void(0)', 'class'=>'button small green', 
						'title'=>Labels::getLabel('LBL_Categories',$adminLangId),"onclick"=>"collectionCategoryForm(".$row['collection_id'].")"),Labels::getLabel('LBL_Categories',$adminLangId), 
					true);
				}
				if( $row['collection_type'] == Collections::COLLECTION_TYPE_SHOP ){
					$innerLiCollectionTypeShop=$innerUl->appendElement('li');					
					$innerLiCollectionTypeShop->appendElement('a', array('href'=>'javascript:void(0)', 'class'=>'button small green', 
				...
				$innerLiDelete->appendElement('a', array('href'=>'javascript:void(0)', 'class'=>'button small green', 
					'title'=>Labels::getLabel('LBL_Delete',$adminLangId),"onclick"=>"deleteRecord(".$row['collection_id'].")"),Labels::getLabel('LBL_Delete',$adminLangId), 
				true);
			}
			break;
