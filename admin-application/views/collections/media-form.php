...
/* collection Images [ */
$imagesHtml = '<div class="row" style="border-bottom:solid 1px #ddd;">Universal<img src="'.CommonHelper::generateUrl('image','collectionReal',array( $collection_id,0, 'THUMB?'.time()),CONF_WEBROOT_FRONT_URL).'">';
...
foreach( $languages as $langId => $langName ){
	...
	$imagesHtml .= '<img src="'.CommonHelper::generateUrl('image','collectionReal',array( $collection_id, $langId, 'THUMB?'.time()),CONF_WEBROOT_FRONT_URL).'">';
...
$bgImagesHtml = '<div class="row" style="border-bottom:solid 1px #ddd;">Universal<img src="'.CommonHelper::generateUrl('image','collectionBgReal',array( $collection_id, 0, 'THUMB?'.time()),CONF_WEBROOT_FRONT_URL).'">';
...
foreach( $languages as $langId => $langName ){
	...
	$bgImagesHtml .= '<img src="'.CommonHelper::generateUrl('image','collectionBgReal',array( $collection_id, $langId, 'THUMB?'.time()),CONF_WEBROOT_FRONT_URL).'">';
...
if( isset($collectionImages) && !empty($collectionImages) ){
	$fld->htmlAfterField = '<div class="uploaded--image"><img src="'.CommonHelper::generateFullUrl('Image','Collection',array($collection_id,'THUMB'),CONF_WEBROOT_FRONT_URL).'?'.time().'"> <a href="javascript:void(0);" onClick="removeCollectionImage('.$collection_id.')" class="remove--img" ><i class="ion-close-round"></i></a></div>';
}
	
$fld = $collectionMediaFrm->getField('collection_bg_image');	
$fld->addFieldTagAttribute('class','btn btn--primary btn--sm');
if( isset($collectionBgImages) && !empty($collectionBgImages) ){
	$fld->htmlAfterField = '<div class="uploaded--image"><img src="'.CommonHelper::generateFullUrl('Image','CollectionBg',array($collection_id,'THUMB'),CONF_WEBROOT_FRONT_URL).'?'.time().'"> <a href="javascript:void(0);" onClick="removeCollectionBGImage('.$collection_id.')" class="remove--img" ><i class="ion-close-round"></i></a></div>';
} */
