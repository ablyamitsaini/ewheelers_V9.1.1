	...
	<div class="fixed_container">
		<div class="row">
		   <div class="space">
				...
				<section class="section">
					<div class="sectionhead">
						<?php 
							...
							$innerLi->appendElement('a', array('href'=>'javascript:window.print()','class'=>'button small green redirect--js no-print','title'=>Labels::getLabel('LBL_Print',$adminLangId)),Labels::getLabel('LBL_Print',$adminLangId), true);	
							echo $ul->getHtml();		
						?>	
					...
					<div class="sectionbody">
						<table class="table table--details">
							...
							<tr>
							  ...
							  <td><strong><?php if($shippingHanldedBySeller){ echo Labels::getLabel('LBL_Delivery/Shipping',$adminLangId); ?>: </strong><?php echo '+'.CommonHelper::displayMoneyFormat(CommonHelper::orderProductAmount($order,'SHIPPING'), true, true);}?> </td>
							</tr>
							<tr>
							  <td><strong><?php if($order['op_tax_collected_by_seller']){ echo Labels::getLabel('LBL_Tax',$adminLangId); ?>: </strong><?php  echo '+'.CommonHelper::displayMoneyFormat(CommonHelper::orderProductAmount($order,'TAX'), true, true); }?>  </td>
							  ...
							  <td><strong><?php echo Labels::getLabel('LBL_Total_Paid',$adminLangId); ?>:</strong><?php echo CommonHelper::displayMoneyFormat(CommonHelper::orderProductAmount($order,'netamount',false,USER::USER_TYPE_SELLER), true, true);?></td>
							</tr>
						</table>
					</div>
				</section>
						...
						<section class="section">
							...
							<div class="row space">
                                <div class="col-lg-6 col-md-6 col-sm-12">
									...
									<p><strong><?php echo Labels::getLabel('LBL_Shop_Name',$adminLangId); ?>:</strong><?php echo $order["op_shop_name"]?><br><strong><?php echo Labels::getLabel('LBL_Name',$adminLangId); ?>: </strong><?php echo $order["op_shop_owner_name"]?><br><strong><?php echo Labels::getLabel('LBL_Email_ID',$adminLangId); ?>:</strong> <?php echo $order["op_shop_owner_email"]?><br><strong><?php echo Labels::getLabel('LBL_Phone',$adminLangId); ?>:</strong> <?php echo $order["op_shop_owner_phone"]?></p>
								</div>
								<div class="col-lg-6 col-md-6 col-sm-12">
									<h5><?php echo Labels::getLabel('LBL_Customer_Details',$adminLangId); ?></h5>
									<p><strong><?php echo Labels::getLabel('LBL_Name',$adminLangId); ?>: </strong><?php echo $order["buyer_name"]?><br><strong><?php echo Labels::getLabel('LBL_UserName',$adminLangId); ?>: </strong><?php echo $order["buyer_username"]; ?><br><strong><?php echo Labels::getLabel('LBL_Email_ID',$adminLangId); ?>:</strong><?php echo $order["buyer_email"]?><br><strong><?php echo Labels::getLabel('LBL_Phone',$adminLangId); ?>:</strong> <?php echo $order["buyer_phone"]?></p>
								</div>
							</div>
						</section>
					</div>
					...
					<div class="sectionbody">
						<table class="table">
							...
							<tr>
								...
								<?php }?>
								<td><?php echo CommonHelper::displayMoneyFormat(CommonHelper::orderProductAmount($order,'VOLUME_DISCOUNT'), true, true);?></td>
						...
						<table class="table">
							...
							<?php 											
								...
								<tr>
									...
									<?php   echo ($row['oshistory_orderstatus_id']>0)?$orderStatuses[$row['oshistory_orderstatus_id']]:CommonHelper::displayNotApplicable($adminLangId,'');  echo ($row['oshistory_tracking_number'])? ': '.Labels::getLabel('LBL_Tracking_Number',$adminLangId).' '.$row['oshistory_tracking_number']." VIA <em>".$row['op_shipping_duration_name']."</em>" :''; ?>
									</td>
								</tr>
							<?php } ?>
					</table>
				...
				<?php if( $displayShippingUserForm ){ ?>
				<section class="section">
					<div class="sectionhead">
