...
class UsersController extends AdminBaseController {	
	...
	public function sendMail(){
		...
		EmailHandler::sendMailTpl($user['credential_email'], 'user_send_email',$this->adminLangId, array(
	...
	public function userRequestsSearch(){
		...
		$srch = new SearchBase(UserRequest::DB_TBL, 'ureq');
		$srch->joinTable(User::DB_TBL, 'LEFT JOIN', 'ureq_user_id = tu.user_id', 'tu');
		$srch->joinTable(User::DB_TBL_CRED, 'LEFT JOIN', 'tuc.credential_user_id = tu.user_id', 'tuc');
	...
	public function truncateUserData()
	{
		...
		if (!$userObj->truncateUserInfo($userData, $userId)) {
			$db->rollbackTransaction();
		...
		if( $userData['user_is_supplier'] || $userData['user_registered_initially_for'] ){
			if (!$userObj->deleteUserReturnAddress($userId)) {
				$db->rollbackTransaction();
