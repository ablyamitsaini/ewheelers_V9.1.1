...
class OptionsController extends AdminBaseController {
	...
	private function getForm($option_id=0){
		...
		$frm->addRequiredField(Labels::getLabel('LBL_OPTION_IDENTIFIER',$adminLangId), 
		'option_identifier');
		
		$languages = Language::getAllNames();
		foreach($languages as $langId=>$langName){
			$fld = $frm->addRequiredField(Labels::getLabel('LBL_OPTION_NAME',$adminLangId).' '.$langName, 
			'option_name'.$langId);
		...
		$frm->addSelectBox(Labels::getLabel('LBL_OPTION_HAVE_SEPARATE_IMAGE',$adminLangId),'option_is_separate_images',
