...
class DiscountCouponsController extends AdminBaseController {
	...
	private function getForm(){
		...
		$frm->addDateField(Labels::getLabel('LBL_Date_To',$this->adminLangId), 'coupon_end_date' , '', array('readonly' => 'readonly','class' => 'small dateTimeFld field--calender'));
	...
	private function getPlanForm(){
		...
		$fld2 = $frm->addHtml('','addNewPlanLink','<a target="_blank" href="'.CommonHelper::generateUrl('sellerPackages').'">'.Labels::getLabel('LBL_Plan_Not_Found?_Click_here_to_add_new_plan',$this->adminLangId).'</a>');
		$fld1->attachField($fld2);
		$frm->addHiddenField('','coupon_id');		
		return $frm;
	}
