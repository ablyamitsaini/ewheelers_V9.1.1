...
class ContentBlockController extends AdminBaseController {
	...
	public function langForm($epage_id = 0,$lang_id = 0){
		...
		} else if($epage_id==Extrapage::ADVERTISER_BANNER_SLOGAN) {
	...
	private function getLangForm($epage_id = 0, $lang_id = 0){
		...
		if(array_key_exists($epage_id,Extrapage::getContentBlockArrWithBg($this->adminLangId)))
		{
			...
			$fld = $frm->addButton(Labels::getLabel('LBL_Backgroud_Image',$this->adminLangId),'cblock_bg_image',Labels::getLabel('LBL_Upload_Image',$this->adminLangId),
	...
	public function setUpBgImage(){
		...
		if(!$res = $fileHandlerObj->saveImage( $_FILES['file']['tmp_name'], $file_type, $epage_id, 0,
