	...
	public function search($type =  false){
		...
		if($type == 'export'){
			...
			while($row = $db->fetch($rs)){				
				$arr = array($row['user_name'],$row['credential_email'], FatDate::format($row['user_regdate']), FatApp::getConfig('conf_currency_symbol').$row['totUserBalance'] );			
				array_push($sheetData,$arr);				
			}
		
			CommonHelper::convertToCsv($sheetData, str_replace("{reportgenerationdate}", date("d-M-Y") , Labels::getLabel("LBL_Advertisers_Report_{reportgenerationdate}",$this->adminLangId)).'.csv', ','); exit;	
