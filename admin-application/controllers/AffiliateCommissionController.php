...
class AffiliateCommissionController extends AdminBaseController {
	...
	public function form( $afcommsetting_id ){
		...
		if ( $afcommsetting_id > 0 ) {
			$data = AffiliateCommission::getAttributesById($afcommsetting_id,
	...
	public function deleteRecord(){
		...
		$row =  AffiliateCommission::getAttributesById($afcommsetting_id,array('afcommsetting_id', 'afcommsetting_is_mandatory') );
		if( $row == false ){
			Message::addErrorMessage( Labels::getLabel("LBL_Invalid_Request", $this->adminLangId ) );
			FatUtility::dieJsonError( Message::getHtml() );	
		}
		...
		FatApp::getDb()->deleteRecords( AffiliateCommission::DB_TBL, array( 'smt' => 'afcommsetting_id = ?', 'vals' => array( $afcommsetting_id )));
