...
class UsersReportController extends AdminBaseController {
	...
	public function search($type =  false){
		...
		*/
		...
		$srch->addMultipleFields(array('u.*','uc.credential_email','COALESCE(tqvo.totVendorOrders,0) as totVendorOrders','COALESCE(tqvo.totSoldQty,0) as totSoldQty','COALESCE(tqvo.totalVendorSales,0) as totalVendorSales','user_name',
		...
		if($type == 'export'){
			...
			while($row = $db->fetch($rs)){				
				$arr = array($row['user_name'],$row['credential_email'], FatDate::format($row['user_regdate']) ,$row['totUserOrderQtys'],$row['totSoldQty'],$row['totUserOrders'],$row['totVendorOrders'],FatApp::getConfig('conf_currency_symbol').$row['totUserOrderPurchases'],FatApp::getConfig('conf_currency_symbol').$row['totalVendorSales'],FatApp::getConfig('conf_currency_symbol').$row['totUserBalance']);			
				array_push($sheetData,$arr);				
			}
		
			CommonHelper::convertToCsv($sheetData, str_replace("{reportgenerationdate}", date("d-M-Y") , Labels::getLabel("LBL_Buyers/Sellers_Report_{reportgenerationdate}",$this->adminLangId)).'.csv', ','); exit;	
