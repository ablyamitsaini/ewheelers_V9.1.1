...
class AffiliatesReportController extends AdminBaseController {
	...
	public function search($type =  false){
		...
		$srch->addMultipleFields(array('u.*','uc.credential_email','user_name',
		...
		if($type == 'export'){
			...
			while($row = $db->fetch($rs)){				
				$arr = array($row['user_name'],$row['credential_email'], FatDate::format($row['user_regdate']), FatApp::getConfig('conf_currency_symbol').$row['totUserBalance'], FatApp::getConfig('conf_currency_symbol').$row['totRevenue'], FatApp::getConfig('conf_currency_symbol').$row['totSignUpRevenue'], FatApp::getConfig('conf_currency_symbol').$row['totOrderRevenue'] );			
				array_push($sheetData,$arr);				
			}
		
			CommonHelper::convertToCsv($sheetData, str_replace("{reportgenerationdate}", date("d-M-Y") , Labels::getLabel("LBL_Affiliates_Report_{reportgenerationdate}",$this->adminLangId)).'.csv', ','); exit;	
